...
def change_sumernote_attchment_model():
    ATTACHMENT_MODEL = "lighting.Image"
...
def get_img_urls(html_content, obj):
    ...
    for image in imgs:
        if image['src'].find("audiotop") != -1:
            ...
            projectObject = models.Produkt.objects.get(slug=obj.slug)
...
class ImageAdmin(AttachmentAdmin):
    ...
    get_foreignkey.short_description = "Produkt"
...
class ProduktAdmin(SummernoteModelAdmin):
